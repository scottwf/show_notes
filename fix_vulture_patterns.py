...
def fix_vulture_patterns():
    # Connect to database
    conn = sqlite3.connect('instance/shownotes.sqlite3')
    db = conn.cursor()
    
    # Generate patterns for Vulture based on the sample URLs
    link_patterns = [
        r'<a[^>]+href="([^"]*)"[^>]*>([^<]*recap[^<]*)</a>',
        r'<a[^>]+href="([^"]*)"[^>]*>([^<]*episode[^<]*)</a>'
    ]
    ...
    # Update Vulture with the patterns
    db.execute("""
        UPDATE recap_sites 
        SET link_patterns = ?, title_patterns = ?, content_patterns = ?
        WHERE site_name = 'Vulture'
    """, (
    ...
    # Verify the update
    result = db.execute("SELECT site_name, link_patterns, title_patterns, content_patterns FROM recap_sites WHERE site_name = 'Vulture'").fetchone()
    print("Updated Vulture patterns:")
